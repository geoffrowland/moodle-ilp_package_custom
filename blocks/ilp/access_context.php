<?php     
    ...
    if (has_capability('block/ilp:tutoreditreview',$coursecontext)) { // are we the tutor on the course ?
    //if (has_capability('block/ilp:view',$usercontext)) { // are we the tutor ? 
    ...
    if (has_capability('block/ilp:supertutoreditreview',$coursecontext)) { // are we the tutor on the course ?
    //if (has_capability('block/ilp:view',$usercontext)) { // are we the tutor ? 

	 $access_issupertutor = 1;
	 //$context = $usercontext;
        //$context = $coursecontext;

    }

    if (!($access_isgod or $access_isuser or $access_isteacher or $access_istutor or $access_issupertutor)) {
