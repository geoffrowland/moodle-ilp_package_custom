...
//////////////////////////////////////////////////
               $mform->addElement('static', 'description', get_string('instructions', 'ilptarget'),
                  get_string('smarttargets', 'ilptarget'));
/////////////////////////////////////////////////////
    ...
    function process_data($data) {
        ...
        $targeturl = $CFG->wwwroot.'/mod/ilptarget/target_view.php'.(($courseid > 1)?'courseid='.$courseid.'&amp;' : '').'&amp;userid='.$userid;
        ...
        if (!$report = get_record('ilptarget_posts', 'id', $targetpost)) {
            ...
            $event->description = $data->targetset.'<br/><a href="'.$CFG->wwwroot.'/mod/ilptarget/target_comments.php?'.(($courseid > 1)?'courseid='.$courseid.'&amp;' : '').'targetpost='.$targetinstance.'">'.$targetview.'</a>';
            ...
            $event->name        = $data->name;
			$event->description = $data->targetset.'<br/><a href="'.$CFG->wwwroot.'/mod/ilptarget/target_comments.php?'.(($courseid > 1)?'courseid='.$courseid.'&amp;' : '').'targetpost='.$report->id.'">'.$targetview.'</a>';
            $event->format      = $data->format;
            $event->timestart   = $data->deadline;
...
function ilptarget_update_status_menu($targetpost,$context) {
    ...
    $courseid = optional_param('courseid', 0, PARAM_INT); //User's targets we wish to view
	...
	if((($age < $CFG->maxeditingtime) && $ownpost) || has_capability('moodle/site:doanything', $context)) {
			$output .= ' | <a title="'.get_string('edit').'" href="'.$CFG->wwwroot.'/mod/ilptarget/target_view.php?'.(($courseid > 1)?'courseid='.$courseid.'&amp;' : '').'userid='.$user->id.'&amp;targetpost='.$report->id.'&amp;action=updatetarget"><img src="'.$CFG->pixpath.'/t/edit.gif" alt="'.get_string('edit').'" /> '.get_string('edit').'</a> | <a title="'.get_string('delete').'" href="'.$CFG->wwwroot.'/mod/ilptarget/target_view.php?'.(($courseid > 1)?'courseid='.$courseid.'&amp;' : '').'userid='.$user->id.'&amp;targetpost='.$report->id.'&amp;action=delete""><img src="'.$CFG->pixpath.'/t/delete.gif" alt="'.get_string('delete').'" /> '.get_string('delete').'</a> | ';
	}
...
class ilptarget_updatecomment_form extends moodleform {
    ...
    function process_data($data) {
        ...
        $targetview = get_string('targetviewlink','ilptarget');
        $commenturl = $CFG->wwwroot.'/mod/ilptarget/target_comments.php?'.(($courseid > 1)?'courseid='.$courseid.'&amp;' : '').'&amp;targetpost='.$targetpost;
...
function ilptarget_update_comment_menu($commentid,$context) {
	...
	if((($age < $CFG->maxeditingtime) && $ownpost) || has_capability('moodle/site:doanything', $context)) {
			$output .= '<a title="'.get_string('edit').'" href="'.$CFG->wwwroot.'/mod/ilptarget/target_comments.php?'.(($courseid > 1)?'courseid='.$courseid.'&amp;' : '').'commentid='.$report->id.'&amp;targetpost='.$report->targetpost.'&amp;action=updatecomment"><img src="'.$CFG->pixpath.'/t/edit.gif" alt="'.get_string('edit').'" /> '.get_string('edit').'</a> | <a title="'.get_string('delete').'" href="'.$CFG->wwwroot.'/mod/ilptarget/target_comments.php?'.(($courseid > 1)?'courseid='.$courseid.'&amp;' : '').'commentid='.$report->id.'&amp;targetpost='.$report->targetpost.'&amp;action=delete"><img src="'.$CFG->pixpath.'/t/delete.gif" alt="'.get_string('delete').'" /> '.get_string('delete').'</a>';
	}
