...
/// Print the main part of the page
	...
	if($id != 0 || $courseid > 0){ //module is accessed through a course use course context 
		...
		if($id != 0) {
			...
			$navigation = "<a href=\"../../course/view.php?id=$course->id\">$course->shortname</a> -> <a href=\"../../blocks/ilp/view.php?courseid=$course->id\">$strilp</a> -> $strtargets";		
			
			print_header("$course->shortname: $strtargets", "$course->fullname",

                 "$navigation", 

                  "", "", true, update_module_button($cm->id, $course->id, $strtargets), 
...
/// and if so, set $currentgroup to reflect the current group
	...
	if (($ausers = get_records_sql($select.$from.$where.$wheresearch.$sort, $table->get_page_start(), $table->get_page_size())) !== false) {
		...
		$navigation = "<a href=\"../targets/view_students.php\">$strtargets</a>";

		

		print_header("$strtargets", "", "$navigation -> ".get_string('mystudents', 'targets')."", 
