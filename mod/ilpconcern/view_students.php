...
/// Print the main part of the page
	...
	if($id != 0 || $courseid > 0){ //module is accessed through a course use course context 
		...
		if($id != 0) {
			...
			print_header("$course->shortname: $concerns->name", "$course->fullname",
			...
			print_header("$course->shortname: $strconcerns", "$course->fullname",
...
/// and if so, set $currentgroup to reflect the current group
	...
	if($CFG->ilpconcern_report5 == 1){
		$tablecolumns[] .= 'report5';
		$tableheaders[] .= '';
	}
	if($CFG->ilpconcern_report6 == 1){
		$tablecolumns[] .= 'report6';
		$tableheaders[] .= '';
	}
	...
	if (($ausers = get_records_sql($select.$from.$where.$wheresearch.$sort, $table->get_page_start(), $table->get_page_size())) !== false) {
			...
			if($CFG->ilpconcern_report5 == 1){
				$report5total = count_records_sql('SELECT COUNT(*) FROM '.$CFG->prefix.'ilpconcern_posts WHERE setforuserid = '.$auser->id.' AND status = 4' );
				$report5recenttotal = count_records_sql('SELECT COUNT(*) FROM '.$CFG->prefix.'ilpconcern_posts WHERE setforuserid = '.$auser->id.' AND status = 4 AND ('.time().' - timecreated) < (7 * 24 * 60 * 60)' );
				if ($report5recenttotal > 0) {
				    $report5recenttotal = '<span style="color: red; font-weight: bold">'.$report5recenttotal.'</span>';
				}
				$report5text  = '<a href="concerns_view.php'.$link_values.'&amp;userid='.$auser->id.'&amp;status=4">'.$report5total.' ('.$report5recenttotal.') '.get_string('report5plural', 'ilpconcern').'</a>';
				$row[] .= $report5text;				
			}
			if($CFG->ilpconcern_report2 == 1){
				$report2total = count_records_sql('SELECT COUNT(*) FROM '.$CFG->prefix.'ilpconcern_posts WHERE setforuserid = '.$auser->id.' AND status = 1' );
				$report2recenttotal = count_records_sql('SELECT COUNT(*) FROM '.$CFG->prefix.'ilpconcern_posts WHERE setforuserid = '.$auser->id.' AND status = 1 AND ('.time().' - timecreated) < (7 * 24 * 60 * 60)' );
				if ($report2recenttotal > 0) {
				    $report2recenttotal = '<span style="color: red; font-weight: bold">'.$report2recenttotal.'</span>';
				}
				$report2text  = '<a href="concerns_view.php'.$link_values.'&amp;userid='.$auser->id.'&amp;status=1">'.$report2total.' ('.$report2recenttotal.') '.get_string('report2plural', 'ilpconcern').'</a>';
				$row[] .= $report2text;				
			}
			if($CFG->ilpconcern_report3 == 1){
				$report3total = count_records_sql('SELECT COUNT(*) FROM '.$CFG->prefix.'ilpconcern_posts WHERE setforuserid = '.$auser->id.' AND status = 2' );
				$report3recenttotal = count_records_sql('SELECT COUNT(*) FROM '.$CFG->prefix.'ilpconcern_posts WHERE setforuserid = '.$auser->id.' AND status = 2 AND ('.time().' - timecreated) < (7 * 24 * 60 * 60)' );
				if ($report3recenttotal > 0) {
				    $report3recenttotal = '<span style="color: green; font-weight: bold">'.$report3recenttotal.'</span>';
				}
				$report3text  = '<a href="concerns_view.php'.$link_values.'&amp;userid='.$auser->id.'&amp;status=2">'.$report3total.' ('.$report3recenttotal.') '.get_string('report3plural', 'ilpconcern').'</a>';
			...
			if($CFG->ilpconcern_report6 == 1){
				$report6total = count_records_sql('SELECT COUNT(*) FROM '.$CFG->prefix.'ilpconcern_posts WHERE setforuserid = '.$auser->id.' AND status = 5' );
				$report6text  = '<a href="concerns_view.php'.$link_values.'&amp;userid='.$auser->id.'&amp;status=5">'.$report6total.' '.get_string('report6plural', 'ilpconcern').'</a>';
				$row[] .= $report6text;				
			}
        ...
        echo '<p style="text-align: right">( ) enclose recent submissions (within the last 7 days)</p>';
		...
		print_header("$strconcerns", "", "$navigation -> ".get_string('mystudents', 'ilpconcern')."", 
