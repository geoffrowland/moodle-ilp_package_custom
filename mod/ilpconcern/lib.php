...
function update_student_status_menu($userid,$courseid) {
	...
	if(!$post = get_record('ilpconcern_status', 'userid',$user->id)) {
		...
		$options = array(get_string('green', 'ilpconcern'), get_string('amber', 'ilpconcern'), get_string('red', 'ilpconcern'),  get_string('contract', 'ilpconcern'));
...
class ilpconcern_updateconcern_form extends moodleform {
    ...
    function definition() {
      ...
      $courseid = $this->_customdata['courseid'];
		...
		//	$mform->addElement('header', 'title', get_string('reportfor', 'ilpconcern', fullname($user)));
                     $mform->addElement('header', 'title', get_string('reportfor', 'ilpconcern',get_string('report'.($status + 1), 'ilpconcern')).fullname($user));
		}
		...
		//
		// Can we set defaults for this ?
		//
		if($concernspost > 0 && $report){
			$mform->setDefault('courserelated', $report->courserelated);
        }
      // Sets Default to course related
      //
      //$mform->setDefault('courserelated', 1);
      //
		...
		// Dropdown of 4 lines
		$mform->addElement('select', 'targetcourse', '<b>Choose Course</b>', $options);
		// $mform->addElement('select', 'targetcourse', '<b>Choose Course</b>', $options);
	...
	function process_data($data) {
      ...
      $courseid = $this->_customdata['courseid'];
		...
		$messagefullname = fullname($messageto);
		////////////////////////////////////////////////////////////////
		$messagetogeoff = get_record('user', 'id', 1);
		$messagetowyn = get_record('user', 'id', 3);
		$messagetosusan = get_record('user', 'id', 12605);
		$messagetoderren = get_record('user', 'id', 12473);
              //$messagetochristine = get_record('user', 'id', 7882);
              //$messagetopamela = get_record('user', 'id', 8083);
              $messagetolaura = get_record('user', 'id', 15538);
		/////////////////////////////////////////////////////////////////
#             $messagetutor = get_record('user', 'id', $tutorid);
...
//////////////////////////////////////////////////////////////////////////
// report status 0 = Old Programme Review - Not Used
// report status 1 = Concern Note
// report status 2 = Recognition Note
// report status 3 = Progress Report - New!
// report status 4 = Attendance Concern
//
                 if($report->status == 1 || $report->status == 2 || $report->status == 4){
		   $message .= ' '.$messagefullname;
			...
			/////////////////////////////////////////////////////////////////////////////////////////
                     //General Concerns to Christine Fells (EMA)
                     //General Concerns to Pamela Spencer (EMA)
                     //General Concerns to Laura Male (EMA)
                     if($report->status == 1) {
			...
			//message_post_message($messagefrom, $messagetochristine, $message, FORMAT_HTML, 'direct');
                     //message_post_message($messagefrom, $messagetopamela, $message, FORMAT_HTML, 'direct');
                     message_post_message($messagefrom, $messagetolaura, $message, FORMAT_HTML, 'direct');
                     }

                     //Attendance Concerns to Attendance Officers 
			if($report->status == 4) {
			//message_post_message($messagefrom, $messagetogeoff, $message, FORMAT_HTML, 'direct');
			//message_post_message($messagefrom, $messagetowyn, $message, FORMAT_HTML, 'direct');
			message_post_message($messagefrom, $messagetosusan, $message, FORMAT_HTML, 'direct');
                     message_post_message($messagefrom, $messagetoderren, $message, FORMAT_HTML, 'direct');
                     // message_post_message($messagefrom, $messagetutor, $message, FORMAT_HTML, 'direct');
                     }
         //////// ///////////////////////////////////////////////////////////////////////////////
                 }
         ////////////////////////////////////////////////////////////////////////////////////////
...
//Creates a drop-down menu to update the status of a concern - for Concerns, Attendance Concern, and Recognitions

function ilpconcern_update_menu($concernpost,$context) {
   ...
   $id = optional_param('id', 0, PARAM_INT); // Course Module ID, or
	...
	$output = '';
   // ($report->status == 0) && has_capability('mod/ilpconcern:editreport1', $context) Review
   // ($report->status == 1) && has_capability('mod/ilpconcern:editreport2', $context) Concern
   // ($report->status == 2) && has_capability('mod/ilpconcern:editreport3', $context) Recognition
   // ($report->status == 3) && has_capability('mod/ilpconcern:editreport4', $context) Report
   // ($report->status == 4) && has_capability('mod/ilpconcern:editreport5', $context) Attendance Concern
   // ($report->status == 5) && has_capability('mod/ilpconcern:editreport6', $context)
   // (($age < $CFG->maxeditingtime) && $ownpost) || 
   //  if((($age < $CFG->maxeditingtime) && $ownpost) ||
   // || has_capability('moodle/site:doanything', $context)
	if(($ownpost) || (($report->status == 0) && has_capability('mod/ilpconcern:editreport1', $context)) || (($report->status == 1) && has_capability('mod/ilpconcern:editreport2', $context)) || (($report->status == 2) && has_capability('mod/ilpconcern:editreport3', $context)) || ($report->status == 3) && has_capability('mod/ilpconcern:editreport4', '1') || (($report->status == 4) && has_capability('mod/ilpconcern:editreport5', $context)) || (($report->status == 5) && has_capability('mod/ilpconcern:editreport6', $context)) || has_capability('moodle/site:doanything', $context)) {
			$output .= ' | <a title="'.get_string('edit').'" href="'.$CFG->wwwroot.'/mod/ilpconcern/concerns_view.php?'.(($courseid > 1)?'courseid='.$courseid.'&amp;' : '').'userid='.$user->id.'&amp;concernspost='.$report->id.'&amp;action=updateconcern&amp;status='.$report->status.'"><img src="'.$CFG->pixpath.'/t/edit.gif" alt="'.get_string('edit').'" /> '.get_string('edit').'</a> | <a title="'.get_string('delete').'" href="'.$CFG->wwwroot.'/mod/ilpconcern/concerns_view.php?'.(($courseid > 1)?'courseid='.$courseid.'&amp;' : '').'userid='.$user->id.'&amp;concernspost='.$report->id.'&amp;action=delete&amp;status='.$report->status.'"><img src="'.$CFG->pixpath.'/t/delete.gif" alt="'.get_string('delete').'" /> '.get_string('delete').'</a> | ';
	}
	
	return $output;
}

//Creates a drop-down menu to update the status of a concern - for Reports

function ilpconcern_report_update_menu($concernpost,$context) {
   ...
   // ($report->status == 3) && has_capability('mod/ilpconcern:editreport4', $context)
   // (($age < $CFG->maxeditingtime) && $ownpost) || 
   //  if((($age < $CFG->maxeditingtime) && $ownpost) ||
   //  if date < Release date
   // || has_capability('moodle/site:doanything', $context)
if(($ownpost) or has_capability('mod/ilpconcern:editreport4', $context) or ($supertutor->id == $USER->id) or has_capability('moodle/site:doanything', $context)) {
			$output .= ' | <a title="'.get_string('edit').'" href="'.$CFG->wwwroot.'/mod/ilpconcern/concerns_view.php?'.(($courseid > 1)?'courseid='.$courseid.'&amp;' : '').'userid='.$user->id.'&amp;concernspost='.$report->id.'&amp;action=updateconcern&amp;status='.$report->status.'"><img src="'.$CFG->pixpath.'/t/edit.gif" alt="'.get_string('edit').'" /> '.get_string('edit').'</a> | <a title="'.get_string('delete').'" href="'.$CFG->wwwroot.'/mod/ilpconcern/concerns_view.php?'.(($courseid > 1)?'courseid='.$courseid.'&amp;' : '').'userid='.$user->id.'&amp;concernspost='.$report->id.'&amp;action=delete&amp;status='.$report->status.'"><img src="'.$CFG->pixpath.'/t/delete.gif" alt="'.get_string('delete').'" /> '.get_string('delete').'</a> | ';
	}
...
function ilpconcern_update_comment_menu($commentid,$context) {
	...
	if((($age < $CFG->maxeditingtime) && $ownpost) || has_capability('mod/ilpconcern:edit', $context) || has_capability('moodle/site:doanything', $context)) {
