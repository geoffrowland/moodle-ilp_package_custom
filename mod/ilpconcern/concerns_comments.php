	...
	if($id != 0){ //module is accessed through a course module use course context 
		...
		print_header("$strconcerns: ".fullname($user)."", "$course->fullname",
                 ...
                 "$navigation -> ".$strconcerns." -> ".fullname($user)."", 
                  ...
                  "", "", true, update_module_button($cm->id, $course->id, $strconcern), 
...
//		echo '<a href="'.$link_values.'&amp;action=updatecomment">'.get_string('addcomment', 'ilpconcern').'</a></div>';
              echo '<a class="button" href="'.$link_values.'&amp;action=updatecomment"" onclick="this.blur();"><span>'.get_string('addcomment', 'ilpconcern').'</span></a>';
              echo '</div>';
